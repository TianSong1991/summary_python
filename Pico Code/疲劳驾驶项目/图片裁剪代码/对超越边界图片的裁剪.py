...
#原始路径
data_root_path = 'I:\\Project_FDS\\original_data\\HX_data\\data1015\\4youshoudadianhua\\picture'

listfiles1 = os.listdir(data_root_path)

#保存路径
data_save_path = 'I:\\rightphone1'
...
for file1 in listfiles1:
    ...
    for json_file_name in json_file_list:
        ...
        random_scale_data = 0.1 * np.random.random() + 1.1  # 1.1到1.2之间
        ...
        if extension1 == '.json':
            ...
            center_point = ((left_up_point_[0] + right_down_point_[0])/2+random_shift_data_x*width,
                           (left_up_point_[1] + right_down_point_[1])/2+random_shift_data_y*width)

            lup_x = int(center_point[0] - int(random_scale_data * width))
            lup_y = int(center_point[1] - int(random_scale_data * width))
            rdp_x = int(center_point[0] + int(random_scale_data * width))
            rdp_y = int(center_point[1] + int(random_scale_data * width))

            left_up_point = (lup_x, lup_y)
            right_down_point = (rdp_x, rdp_y)
            ...
            if (left_up_point[0] < 0 or left_up_point[1] < 0 or right_down_point[0] > 640 - 1 or right_down_point[1] > 480 - 1):
                ...
                center_point = (center_point[0]+range, center_point[1]+range)
                left_up_point = (int(center_point[0] - int(random_scale_data * width)),int(center_point[1] - int(random_scale_data * width)))
                right_down_point = (int(center_point[0] + int(random_scale_data * width)),int(center_point[1] + int(random_scale_data * width)))
                #print(width)
